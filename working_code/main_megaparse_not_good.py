...
def extract_info_from_pdf(file_path: str) -> Optional[ResumeAnalysis]:
    """
    Extract and analyze information from a PDF resume
    Returns None if analysis fails
    """
    try:
        print(f"Extracting text from: {file_path}")
        # Extract text using MegaParse - initialize with file path
...
def main():
    ...
    try:
        ...
        # Save analyses if we have any successful analyses
        if all_analyses:
            save_analysis_to_markdown(all_analyses, markdown_file)
            print(f"\nAnalyses saved to {markdown_file}")
