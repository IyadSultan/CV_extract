...
def extract_info_from_pdf(file_path: str) -> Optional[ResumeAnalysis]:
    ...
    try:
        ...
        return analyze_resume_text(cleaned_text)

    except Exception as e:
        print(f"Error processing PDF: {str(e)}")
        return None

def extract_info_from_docx(file_path: str) -> Optional[ResumeAnalysis]:
    """
    Extract and analyze information from a Word document resume
    Returns None if analysis fails
    """
    try:
        print(f"Extracting text from Word document: {file_path}")
...
def analyze_resume_text(cleaned_text: str) -> Optional[ResumeAnalysis]:
    """
    Analyze the cleaned resume text using the resume agent
    Returns None if analysis fails
    """
    max_retries = 3
...
def main():
    ...
    markdown_file = 'results/resume_analysis_results.md'
    csv_file = 'results/resume_analysis_results.csv'
    ...
    try:
        ...
        for filename in os.listdir(cv_directory):
            ...
            if filename.lower().endswith('.pdf'):
                analysis = extract_info_from_pdf(file_path)
            elif filename.lower().endswith(('.docx', '.doc')):
                analysis = extract_info_from_docx(file_path)
            else:
